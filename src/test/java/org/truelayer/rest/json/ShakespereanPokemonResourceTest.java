...
public class ShakespereanPokemonResourceTest {
    ...
    public void testPokemonNotFound() {
		...
		String aPath = classLoader.getResource(fileName).getFile();
		LOGGER.info(aPath);
		testPokemon(aPokemonName, aPath, 500);
    }
    ...
    public void testPokemon(String pokemonName, String expectedReplyPath, Integer expectedStatusCode) {
		...
		try {
			...
			ShakespeareanPokemon aShakespereanPokemon = objectMapper.readValue(file, ShakespeareanPokemon.class);
			aExpectedReply = objectMapper.writeValueAsString(aShakespereanPokemon);
			LOGGER.info(aExpectedReply);
		} catch (IOException e) {
			e.printStackTrace();
		}
